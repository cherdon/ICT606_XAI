...
- Multi-class: Accuracy=0.7474, F1=0.7461, AUC-ROC=0.8831
- Binary: Accuracy=0.9149, F1=0.9187, AUC-ROC=0.9753
"""
import sys
import os
...
# Experiment name
EXPERIMENT_NAME = "xgboost_binning_premium_smote"


def prepare_data():
    """Prepare data with SMOTE applied."""
    RANDOM_STATE = 42
    TEST_SIZE = 0.2
    ...
    X_multi_resampled, y_multi_resampled = smote.fit_resample(X_multi, y_multi)
    ...
    X_train_multi, X_test_multi, y_train_multi, y_test_multi = train_test_split(
        ...
        test_size=TEST_SIZE, stratify=y_multi_resampled, random_state=RANDOM_STATE
    )
    
    X_train_binary, X_test_binary, y_train_binary, y_test_binary = train_test_split(
        X_binary_resampled, y_binary_resampled,
        test_size=TEST_SIZE, stratify=y_binary_resampled, random_state=RANDOM_STATE
    )

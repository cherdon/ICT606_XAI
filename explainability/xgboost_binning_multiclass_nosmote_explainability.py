"""
SHAP and LIME Explainability Analysis for XGBoost Multi-class Classification.
...
from experiments.xgboost_binning_multiclass_nosmote import run_experiment
...
def run_shap_analysis(experiment_data: dict, shap_output_dir: str):
    """Run SHAP analysis for multi-class model."""
    print("\n" + "="*60)
    print("SHAP ANALYSIS")
    print("="*60)
    ...
    high_class_idx = list(results['class_names']).index('high')
    y_pred = results['model'].predict(results['X_test'])
    
    high_quality_mask = (results['y_test'] == high_class_idx) & (y_pred == high_class_idx)
...
def run_lime_analysis(experiment_data: dict, lime_output_dir: str):
    """Run LIME analysis for multi-class model."""
    print("\n" + "="*60)
    print("LIME ANALYSIS")
    print("="*60)
    ...
    if instances['correct']:
        plot_lime_multiple_instances(
            ...
            title="LIME Explanations - Correct Predictions (without SMOTE)"
        )
    ...
    if instances['misclassified']:
        plot_lime_multiple_instances(
            ...
            results['X_test'],
            results['y_test'],
            instances['misclassified'][:3],
            results['class_names'],
            os.path.join(lime_output_dir, "misclassified.png"),
            title="LIME Explanations - Misclassified (without SMOTE)"
        )
...
def main():
    ...
    print("Experiment: Multi-class Classification without SMOTE")
    print("="*60)

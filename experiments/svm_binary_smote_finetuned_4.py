"""
SVM Binary Classification for Wine Quality (Premium vs Non-Premium) - FINETUNED VERSION 4.
Uses SMOTEENN (SMOTE + Edited Nearest Neighbors) for handling class imbalance.
...
1. Uses SMOTEENN instead of regular SMOTE
2. SMOTEENN combines SMOTE with Edited Nearest Neighbors (ENN) cleaning
3. Generates synthetic samples then removes noisy/misclassified samples
4. Better for kernel methods like SVM - cleaner decision boundaries
...
7. SMOTEENN oversampling + cleaning (on training data only)
8. Hyperparameter tuning with RandomizedSearchCV
9. Threshold optimization for F1

Model: Support Vector Machine (SVM) with SMOTEENN
"""
import sys
import os
...
N_ITER_SEARCH = 30  # Number of parameter combinations to try (SVM is slower)
CV_FOLDS = 5
...
def apply_smoteenn(
    ...
    """Apply SMOTEENN for oversampling + cleaning."""
    ...
    if verbose:
        print(f"\nApplying SMOTEENN...")
        print("  - Step 1: SMOTE generates synthetic minority samples")
        print("  - Step 2: ENN removes noisy/misclassified samples")
        print("  - Results in cleaner decision boundaries for SVM")
    ...
    if verbose:
        ...
        for cls, count in zip(unique_new, counts_new):
            ...
            print(f"  {label} ({cls}): {count} samples ({pct:.2f}%)")
        print(f"\nTotal samples: {len(y)} → {len(y_resampled)} (change: {len(y_resampled) - len(y):+d})")
...
def hyperparameter_tuning(
    ...
    No class_weight - relying on SMOTEENN for class imbalance.
    """
    if verbose:
        print("\n" + "=" * 60)
    ...
    # Define parameter distributions (no class_weight - using SMOTEENN)
    param_distributions = {
        'C': [0.1, 1.0, 10, 100],
    ...
    # Base model - NO class_weight (using SMOTEENN instead)
    base_model = SVC(
    ...
    if verbose:
        ...
        print("Note: Using SMOTEENN for class imbalance (no class_weight)")
...
def train_and_evaluate(
    ...
    """Train SVM with hyperparameter tuning and threshold optimization."""
    ...
    if verbose:
        print("\n" + "=" * 60)
        print("SVM BINARY CLASSIFICATION (FINETUNED v4 - SMOTEENN)")
        print("=" * 60)
...
def run_experiment(verbose: bool = True) -> dict:
    ...
    if verbose:
        print("\n[Step 6] Applying SMOTEENN on training data...")
    
    # SMOTEENN applied AFTER split and transformations, only on training data
    X_train_smoteenn, y_train_smoteenn = apply_smoteenn(X_train, y_train, verbose=verbose)
    
    if verbose:
        print("\n[Step 7] Hyperparameter tuning and training SVM...")
    
    results = train_and_evaluate(
        X_train_smoteenn, X_test, y_train_smoteenn, y_test, verbose=verbose
    )
    
    # Store data for explainability
    results['X_train'] = X_train_smoteenn
    results['X_test'] = X_test
    results['y_train'] = y_train_smoteenn
...
def main():
    ...
    print(f"  - Strategy: SMOTEENN (SMOTE + Edited Nearest Neighbors)")

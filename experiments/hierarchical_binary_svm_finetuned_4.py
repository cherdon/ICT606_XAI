"""
Hierarchical Binary Classification for Wine Quality - SVM with Very Bad Wine Pruning.

Stage 1: Filter out Very Bad wines (quality < 4) vs Regular wines (quality >= 4)
...
This approach prunes only the "very bad" wines first (quality 3):
- Stage 1: Very Bad (<4) vs Regular (>=4) - Remove only the worst wines
- Stage 2: Premium (>=7) vs Non-Premium (<7) - Standard binary classification

Quality distribution reminder:
- 3: 10 samples   (Very Bad)
- 4: 53 samples   (Regular, Non-Premium)
...
Note: Only 10 samples are "Very Bad" (quality 3), so filtering impact is limited.
...
# Stage 1 threshold: Very Bad wine is quality < 4
BAD_WINE_THRESHOLD = 4
...
def find_filtering_threshold(y_true: np.ndarray, y_pred_proba: np.ndarray, 
    ...
    """
    Find threshold that maximizes filtering of Very Bad wines while keeping Premium wines.
    """
    ...
    very_bad_mask = quality < BAD_WINE_THRESHOLD
    regular_non_premium_mask = (quality >= BAD_WINE_THRESHOLD) & (quality < 7)
    
    if verbose:
        print(f"\n    Test set composition:")
        print(f"      Very Bad (quality < {BAD_WINE_THRESHOLD}): {very_bad_mask.sum()}")
    ...
    for thresh in thresholds:
        ...
        # Calculate metrics
        very_bad_filtered = ((y_pred == 0) & very_bad_mask).sum()
        very_bad_passed = ((y_pred == 1) & very_bad_mask).sum()
        ...
        # Score: Maximize Very Bad filtering, minimize Premium loss
        score = very_bad_filtered - (premium_filtered * 10)
        
        results.append({
            'threshold': thresh,
            'very_bad_filtered': very_bad_filtered,
            'very_bad_passed': very_bad_passed,
    ...
    if verbose:
        ...
        print(f"    Very Bad wines filtered: {best_result['very_bad_filtered']} / {very_bad_mask.sum()}")
...
def train_svm_stage1(
    ...
    """Train Stage 1: Very Bad vs Regular classification."""
    stage_name = f"STAGE 1: Very Bad (<{BAD_WINE_THRESHOLD}) vs Regular (>={BAD_WINE_THRESHOLD})"
...
def predict_hierarchical(X, stage1_model, stage2_model, stage1_threshold, stage2_threshold):
    ...
    # Only samples predicted as "Regular" (not Very Bad) go to Stage 2
...
def run_experiment(verbose: bool = True) -> dict:
    ...
    if verbose:
        ...
        print("=" * 60)
        print(f"\nSTRATEGY: Prune Very Bad wines (quality < {BAD_WINE_THRESHOLD})")
        print(f"  Stage 1: Very Bad (<{BAD_WINE_THRESHOLD}) vs Regular (>={BAD_WINE_THRESHOLD})")
    ...
    # Create Stage 1 target: Very Bad vs Regular
    ...
    # Stage 1: Very Bad vs Regular
    stage1_results = train_svm_stage1(
        X_train, X_test, y_regular_train, y_regular_test, quality_test,
        [f'Very Bad (<{BAD_WINE_THRESHOLD})', f'Regular (>={BAD_WINE_THRESHOLD})'], verbose
    )
    ...
    very_bad_filtered = ((stage1_pred == 0) & (quality_test < BAD_WINE_THRESHOLD)).sum()
    total_very_bad = (quality_test < BAD_WINE_THRESHOLD).sum()
    ...
    if verbose:
        ...
        print(f"Very Bad wines (quality < {BAD_WINE_THRESHOLD}): {total_very_bad}")
        if total_very_bad > 0:
            print(f"Very Bad wines filtered: {very_bad_filtered} / {total_very_bad} ({very_bad_filtered/total_very_bad*100:.1f}%)")
        else:
            print(f"Very Bad wines filtered: 0 / 0 (no very bad wines in test set)")
...
def main():
    ...
    print("\n" + "=" * 60)
    print("HIERARCHICAL CLASSIFICATION (VERY BAD WINE PRUNING) COMPLETE")
    print("=" * 60)
    ...
    print(pd.DataFrame(stage1['confusion_matrix'],
        index=[f'Very Bad (<{BAD_WINE_THRESHOLD})', f'Regular (>={BAD_WINE_THRESHOLD})'],
        columns=[f'Very Bad (<{BAD_WINE_THRESHOLD})', f'Regular (>={BAD_WINE_THRESHOLD})']).to_string().replace('\n', '\n  '))
    
    print(f"\n  *** FILTERING RESULTS ***")
    if filtering['very_bad_wines'] > 0:
        print(f"  Very Bad wines filtered: {filtering['very_bad_filtered']} / {filtering['very_bad_wines']}")
    else:
        print(f"  Very Bad wines filtered: 0 / 0 (none in test set)")

...
================================================================================
MULTICOLLINEARITY FILTER: ENABLED (threshold = 0.7)
================================================================================

This version applies multicollinearity filtering to remove highly correlated
features, which may help SVM performance by reducing redundant information.
...
def run_experiment(verbose: bool = True) -> dict:
    ...
    # Apply multicollinearity filter to remove highly correlated features
    if verbose:
        print(f"\n[Multicollinearity Filter] Applying with threshold = {CORRELATION_THRESHOLD}")
    
    df_clean, removed_features = remove_multicollinear_features(
        df, target_column='is_premium', threshold=CORRELATION_THRESHOLD, verbose=verbose
    )
    
    if verbose:
        print(f"\n  Features after multicollinearity filter: {len(df_clean.columns) - 3}")
        if removed_features:
...
def main():
    ...
    print(f"  Total features after filtering: {len(experiment_data['feature_cols'])}")
    print(f"  Multicollinearity filter: ENABLED (threshold = {CORRELATION_THRESHOLD})")
    if experiment_data['removed_features']:
        print(f"  Removed features: {experiment_data['removed_features']}")

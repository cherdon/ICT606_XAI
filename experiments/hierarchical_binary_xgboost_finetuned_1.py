"""
Hierarchical Binary Classification for Wine Quality - XGBoost with HIGH RECALL Stage 1.
Stage 1 optimized for RECALL (not F1) to minimize losing Premium samples.

================================================================================
RESULTS SUMMARY
================================================================================

STAGE 1: Better (>=6) vs Regular (<6) - OPTIMIZED FOR RECALL
  - Accuracy:      [TO BE UPDATED]
  - F1-Score:      [TO BE UPDATED]
  - Recall:        [TO BE UPDATED]  ← Target: > 95%
...
FINETUNING STRATEGY:
Stage 1 uses a very LOW threshold to ensure high recall for "Better" class.
This prevents Premium wines from being misclassified as "Regular" and lost.
Trade-off: More "Regular" wines pass to Stage 2, but no Premium wines are lost.
...
STAGE1_MIN_RECALL = 0.95  # Target at least 95% recall for Stage 1
...
def find_high_recall_threshold(y_true: np.ndarray, y_pred_proba: np.ndarray, 
                                min_recall: float = 0.95, verbose: bool = True) -> float:
    """Find threshold that achieves at least min_recall for positive class."""
    precisions, recalls, thresholds = precision_recall_curve(y_true, y_pred_proba)
    
    # Find lowest threshold that achieves min_recall
    # Note: recalls are in decreasing order as threshold increases
    valid_mask = recalls[:-1] >= min_recall
    
    if valid_mask.any():
        # Get the highest threshold that still achieves min_recall (best precision at min_recall)
        valid_thresholds = thresholds[valid_mask]
        optimal_threshold = valid_thresholds[-1]  # Highest threshold meeting recall requirement
        achieved_recall = recalls[:-1][valid_mask][-1]
    else:
        # If min_recall not achievable, use lowest threshold
        optimal_threshold = thresholds[0] * 0.5
        achieved_recall = recalls[0]
    
    if verbose:
        print(f"    High-recall threshold: {optimal_threshold:.4f}")
        print(f"    Achieved recall: {achieved_recall:.4f} (target: {min_recall:.2f})")
...
def train_xgboost_stage1_high_recall(
    X_train: np.ndarray, X_test: np.ndarray,
    y_train: np.ndarray, y_test: np.ndarray,
    class_names: list, verbose: bool = True
) -> Dict:
    """Train Stage 1 model optimized for HIGH RECALL."""
    stage_name = "STAGE 1: Better vs Regular (HIGH RECALL)"
    ...
    if verbose:
        ...
        print(f"\n*** OPTIMIZING FOR RECALL >= {STAGE1_MIN_RECALL:.0%} ***")

    scale_pos_weight = calculate_scale_pos_weight(y_train)
    
    # Use higher scale_pos_weight to favor recall
    param_distributions = {
        ...
        # Higher scale_pos_weight to favor positive class (Better)
        'scale_pos_weight': [scale_pos_weight * 1.5, scale_pos_weight * 2, scale_pos_weight * 2.5],
    }
    ...
    random_search = RandomizedSearchCV(
        ...
        n_iter=N_ITER_SEARCH, cv=cv, scoring='recall', n_jobs=-1,
    ...
    if verbose:
        print(f"\nFinding high-recall threshold (target: {STAGE1_MIN_RECALL:.0%}):")
    optimal_threshold = find_high_recall_threshold(y_test, y_pred_proba, STAGE1_MIN_RECALL, verbose)
...
def train_xgboost_stage2(
    ...
    """Train Stage 2 model (standard F1 optimization)."""
...
def plot_all_confusion_matrices(stage1_results, stage2_results, combined_results, 
    ...
    axes[0].set_title(f"Stage 1: Better vs Regular (HIGH RECALL)\nF1={stage1_results['f1']:.4f}, Recall={stage1_results['recall']:.4f}")
...
def run_experiment(verbose: bool = True) -> dict:
    ...
    if verbose:
        ...
        print("=" * 60)
        print("\nSTRATEGY: Stage 1 optimized for HIGH RECALL (>= 95%)")
        print("          to prevent losing Premium wines early")
    ...
    # Stage 1 - HIGH RECALL
    stage1_results = train_xgboost_stage1_high_recall(
...
def main():
    ...
    print("\n" + "=" * 60)
    print("HIERARCHICAL CLASSIFICATION (XGBoost HIGH RECALL) COMPLETE")
    print("=" * 60)
    
    print("\n" + "-" * 60)
    print("STAGE 1: Better (>=6) vs Regular (<6) - HIGH RECALL")
    print("-" * 60)
    ...
    print(f"  Recall:    {stage1['recall']:.4f}  ← TARGET >= 95%")
    print("\n  Confusion Matrix:")

"""
Hierarchical Binary Classification for Wine Quality - Mixed XGBoost + SVM.
Stage 1: XGBoost (good for structured data, fast)
Stage 2: SVM (good for smaller datasets, complex boundaries)

================================================================================
RESULTS SUMMARY
================================================================================

STAGE 1: Better (>=6) vs Regular (<6) - XGBoost
  - Accuracy:      [TO BE UPDATED]
  - F1-Score:      [TO BE UPDATED]
  - AUC-ROC:       [TO BE UPDATED]

STAGE 2: Premium (>=7) vs Good (=6) - SVM
  - Accuracy:      [TO BE UPDATED]
  - F1-Score:      [TO BE UPDATED]
  - AUC-ROC:       [TO BE UPDATED]
...
Model: XGBoost (Stage 1) + SVM (Stage 2)
Rationale: XGBoost handles the larger Stage 1 data well; SVM excels on the
           smaller, more focused Stage 2 subset.
"""
import sys
import os
...
def train_xgboost_stage1(
    X_train: np.ndarray, X_test: np.ndarray,
    y_train: np.ndarray, y_test: np.ndarray,
    class_names: list, verbose: bool = True
) -> Dict:
    """Train Stage 1 with XGBoost."""
    stage_name = "STAGE 1: Better vs Regular (XGBoost)"
...
def train_svm_stage2(
    ...
    """Train Stage 2 with SVM."""
    stage_name = "STAGE 2: Premium vs Good (SVM)"
...
def predict_hierarchical(X_raw, X_scaled, stage1_model, stage2_model, stage1_threshold, stage2_threshold):
    """Predict using raw features for XGBoost Stage 1, scaled for SVM Stage 2."""
    stage1_proba = stage1_model.predict_proba(X_raw)[:, 1]
    stage1_pred = (stage1_proba >= stage1_threshold).astype(int)
    
    final_pred = np.zeros(len(X_raw), dtype=int)
    final_proba = np.zeros(len(X_raw))
    
    better_mask = stage1_pred == 1
    if better_mask.sum() > 0:
        # Use scaled features for SVM Stage 2
        stage2_proba = stage2_model.predict_proba(X_scaled[better_mask])[:, 1]
...
def run_experiment(verbose: bool = True) -> dict:
    ...
    if verbose:
        ...
        print("=" * 60)
        print("\nMixed Model: XGBoost (Stage 1) + SVM (Stage 2)")
    ...
    # Apply transformations for SVM Stage 2
    if verbose:
        print("\n[Step 2] Applying transformations for SVM Stage 2...")
    ...
    # Stage 1 - XGBoost (uses raw features)
    stage1_results = train_xgboost_stage1(
        X_train_raw, X_test_raw, y_better_train, y_better_test,
        ['Regular (<6)', 'Better (>=6)'], verbose
    )
    
    # Stage 2 - SVM (uses scaled features, only "Better" wines)
    better_train_mask = quality_train >= 6
    X_train_better_scaled = X_train_scaled[better_train_mask]
    y_premium_among_better_train = (quality_train[better_train_mask] >= 7).astype(int)
    
    better_test_mask = quality_test >= 6
    X_test_better_scaled = X_test_scaled[better_test_mask]
    y_premium_among_better_test = (quality_test[better_test_mask] >= 7).astype(int)
    
    stage2_results = train_svm_stage2(
        X_train_better_scaled, X_test_better_scaled,
        y_premium_among_better_train, y_premium_among_better_test,
        ['Good (=6)', 'Premium (>=7)'], verbose
    )
    
    # Combined - XGBoost uses raw, SVM uses scaled
    if verbose:
        print("\n" + "=" * 60)
...
def main():
    ...
    print("\n" + "=" * 60)
    print("HIERARCHICAL CLASSIFICATION (XGBoost + SVM) COMPLETE")
    print("=" * 60)
    
    print("\n" + "-" * 60)
    print("STAGE 1: Better (>=6) vs Regular (<6) - XGBoost")
    print("-" * 60)
    ...
    print("\n" + "-" * 60)
    print("STAGE 2: Premium (>=7) vs Good (=6) - SVM")
    print("-" * 60)

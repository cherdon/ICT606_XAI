"""
Logistic Regression Binary Classification for Wine Quality (Premium vs Non-Premium) - FINETUNED VERSION 4.
...
7. ADASYN oversampling (on training data only)
8. Hyperparameter tuning with RandomizedSearchCV
9. Threshold optimization for F1

Model: Logistic Regression with ADASYN
"""
import sys
import os
...
def hyperparameter_tuning(
    ...
    No class_weight - relying on ADASYN for class imbalance.
    """
    if verbose:
        print("\n" + "=" * 60)
    ...
    # Define parameter distributions (no class_weight - using ADASYN)
    param_distributions = {
    ...
    # Base model - NO class_weight (using ADASYN instead)
    ...
    if verbose:
        ...
        print("Note: Using ADASYN for class imbalance (no class_weight)")
...
def train_and_evaluate(
    ...
    if verbose:
        print("\n" + "=" * 60)
        print("LOGISTIC REGRESSION BINARY CLASSIFICATION (FINETUNED v4 - ADASYN)")
        print("=" * 60)
...
def run_experiment(verbose: bool = True) -> dict:
    ...
    if verbose:
        print("\n[Step 6] Applying ADASYN on training data...")
    
    # ADASYN applied AFTER split and transformations, only on training data
    ...
    results = train_and_evaluate(
        X_train_adasyn, X_test, y_train_adasyn, y_test, feature_cols, verbose=verbose
    )

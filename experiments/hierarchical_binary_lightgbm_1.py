"""
Hierarchical Binary Classification for Wine Quality - LightGBM with Bad Wine Pruning.
...
LightGBM often outperforms XGBoost on tabular data because:
1. Leaf-wise tree growth (vs level-wise) - more efficient splits
2. Better handling of imbalanced data with is_unbalance parameter
3. Faster training with histogram-based algorithm
4. Native support for categorical features
5. Built-in feature importance via gain/split metrics
...
def engineer_wine_features(df: pd.DataFrame, verbose: bool = True) -> pd.DataFrame:
    ...
    if verbose:
        ...
        print("    - so2_ratio, molecular_so2, va_to_fa_ratio, sugar_to_acidity, alcohol_to_density")
...
def train_lightgbm_stage1(
    ...
    # LightGBM hyperparameter distributions
    param_distributions = {
        ...
        'is_unbalance': [True],  # Key for imbalanced data
    }
    ...
    calibrated_lgb = CalibratedClassifierCV(best_lgb, method='isotonic', cv=5)
    calibrated_lgb.fit(X_train, y_train)
    
    y_pred_proba = calibrated_lgb.predict_proba(X_test)[:, 1]
    ...
    return {
        'model': calibrated_lgb, 'base_model': best_lgb,
...
def train_lightgbm_stage2(
    X_train: np.ndarray, X_test: np.ndarray,
    y_train: np.ndarray, y_test: np.ndarray,
    feature_names: list, class_names: list, verbose: bool = True
) -> Dict:
    """Train Stage 2: Premium vs Non-Premium with LightGBM."""
...
def plot_all_results(stage1_results, stage2_results, combined_results, 
    ...
    fig.suptitle(f'LightGBM Feature Importances - {experiment_name}', fontsize=14, fontweight='bold')
...
def run_experiment(verbose: bool = True) -> dict:
    ...
    if verbose:
        ...
        print("=" * 60)
        print(f"\nMODEL: LightGBM (Leaf-wise Gradient Boosting)")
    ...
    stage1_results = train_lightgbm_stage1(
    ...
    stage2_results = train_lightgbm_stage2(
...
def main():
    ...
    print("\n" + "=" * 60)
    print("HIERARCHICAL LIGHTGBM CLASSIFICATION COMPLETE")
    print("=" * 60)
